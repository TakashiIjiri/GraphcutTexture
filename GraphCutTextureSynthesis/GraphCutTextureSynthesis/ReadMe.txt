================================================================================
    MFC (MICROSOFT FOUNDATION CLASS) ライブラリ : GraphCutTextureSynthesis プロジェクトの概要
===============================================================================

この GraphCutTextureSynthesis アプリケーションは、アプリケーション ウィザードによって作成されました。このアプリケーションは MFC の基本的な使い方を示すだけでなく、アプリケーションを作成するための開始点でもあります。

このファイルには、GraphCutTextureSynthesis アプリケーションを構成する各ファイルの内容の概要が含まれています。

GraphCutTextureSynthesis.vcxproj
...
GraphCutTextureSynthesis.vcxproj.filters
...
GraphCutTextureSynthesis.h
    これはアプリケーションのメイン ヘッダー ファイルです。
    その他のプロジェクト固有のヘッダー (Resource.h など) が含まれ、CGraphCutTextureSynthesisApp アプリケーション クラスを宣言します。

GraphCutTextureSynthesis.cpp
    これはメインのアプリケーション ソース ファイルで、CGraphCutTextureSynthesisApp アプリケーション クラスが含まれています。

GraphCutTextureSynthesis.rc
...
res\GraphCutTextureSynthesis.ico
    これはアプリケーションのアイコンとして使用されるアイコン ファイルです。このアイコンは、メイン リソース ファイル GraphCutTextureSynthesis.rc にインクルードされます。

res\GraphCutTextureSynthesis.rc2
...
res\Toolbar.bmp
    ...
    初期ツール バーと初期ステータス バーは、CMainFrame クラスで生成されます。ツール バーのボタンを追加するには、リソース エディターを使用してこのツール バーのビットマップを編集し、GraphCutTextureSynthesis.rc 内の IDR_MAINFRAME TOOLBAR 配列を更新します。
/////////////////////////////////////////////////////////////////////////////

アプリケーション ウィザードは 1 つのドキュメントの種類と 1 つのビューを作成します。

GraphCutTextureSynthesisDoc.h, GraphCutTextureSynthesisDoc.cpp - ドキュメント
    これらのファイルには、CGraphCutTextureSynthesisDoc クラスが含まれます。これらのファイルを編集して、特殊なドキュメント データを追加し、ファイルの保存と読み込みを実装することができます (CGraphCutTextureSynthesisDoc::Serialize を使用)。

GraphCutTextureSynthesisView.h, GraphCutTextureSynthesisView.cpp - ドキュメントのビュー
    これらのファイルには、CGraphCutTextureSynthesisView クラスが含まれます。
    CGraphCutTextureSynthesisView オブジェクトは CGraphCutTextureSynthesisDoc オブジェクトを表示するために使用されます。
...
StdAfx.h, StdAfx.cpp
    これらのファイルは、GraphCutTextureSynthesis.pch という名前のプリコンパイル済みヘッダー (PCH) ファイルと、StdAfx.obj という名前のプリコンパイル済みの型ファイルをビルドするために使用されます。

Resource.h
    これは、新しいリソース ID を定義する標準のヘッダー ファイルです。このファイルの読み込みおよび更新は、Microsoft Visual C++ で行います。

GraphCutTextureSynthesis.manifest
